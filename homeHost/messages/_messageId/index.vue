<template>
  <div>
    <HostHeader :userProfilesData="haveUserData" />
    <div>
        ...
        <div class="row app-one">
          <div class="col-sm-4 side">
            <div class="side-one">
              ...
              <div class="row sideBar" id="style-1" v-if="userData.length != 0">
                <div
                  class="row sideBar-body"
                  v-for="user in allSearchData"
                  :key="user.firebaseId"
                  @click.prevent="changeChat(user.authId)"
                  :id="user.authId == guestId ? 'choseBorder' : 'choseBorders'"
                >
                  <div class="col-sm-2 col-xs-3 sideBar-avatar">
                    <div class="avatar-icon">
                      <img
                        :src="
                          user.authData.profileImage == null
                            ? './user.png'
                            : user.authData.profileImage
                        "
                        class="mt-3"
                        style="object-fit: cover"
                        v-if="user.authData.profileImage"
                      />
                      <img src="./user.png" style="object-fit: cover" v-else />
                    </div>
                  </div>
                    ...
                    <div class="row">
                      <div class="col-sm-6 h-25 col-xs-8 sideBar-name">
                        <span class="name-meta" style="margin-left: -4px"
                          >{{
                            user.authData.fullName == null
                              ? "İsim belirtilmemiş"
                              : user.authData.fullName
                          }}
                        </span>
                      </div>
                      <div
                      ...
                      <div
                        class="col-12 small h-50 ml-2 w-100"
            ...
            <div class="row heading">
              <div class="col-sm-2 col-md-1 col-xs-3 heading-avatar">
                ...
                >
                  <img
                    class="photo-object"
                    :src="chooseProfile[0].authData.profileImage"
                    alt=""
                  />
                </div>
              </div>
              ...
              >
                <a class="heading-name-meta"
                  >{{ chooseProfile[0].authData.fullName == undefined ? 'İsim yok' : chooseProfile[0].authData.fullName  }}
                </a>
              </div>
              <div
                v-if="chooseProfile.length != 0"
      ...
      <div class="container w-100  app font chat-mobile" style="margin-top:-35px">
          ...
          <div class="side-one ">
            <div class="row heading">
              ...
              >
                  ...
                  {{ chooseProfile[0].authData.fullName }}
               
              </div>
              <div class="col-sm-10 col-xs-7 heading-name" v-else>İsim Yok</div>
              </div>
        ...
        <div v-if="conversation">
         ...
         <div class="row message scroll h-100" id="chat-history2" ref="msg_history">
              <div
                class="row message-body mb-0"
                v-for="message in chatMessages"
                :key="message.id"
             ...
             <div class="row reply">
              ...
              <div
                ...
                @click.prevent="saveMessage"
              >
                <i class="fas fa-paper-plane" style="font-size:25px;margin-top:25%;margin-left:40%;color:#f93b3b"></i>
              </div>
            </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    changeChat(guestId) {
      ...
      let ref = this.$fire.firestore.collection(guestId);
    ...
    saveMessage() {
      if (this.message != null) {
        let ref = this.$fire.firestore.collection(
          this.chooseProfile[0].authData.authId
        );
        ref.doc(this.$store.state.authKey).collection("chat").doc().set({
    ...
    searchUsers() {
      if (this.searchUser != null) {
        this.allSearchData = this.userData.filter((s) =>
          s.authData.fullName.startsWith(this.searchUser)
        );
      }
    },
    chanceSide() {
  ...
  created() {
    this.getProfile();
    this.getRezervData();
    this.allSearchData = this.userData;
  },
};
</script>
...
#choseBorder {
  border-left: 5px solid #f93b3b !important;
}
